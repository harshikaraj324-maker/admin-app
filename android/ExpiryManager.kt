...
class ExpiryManager {
    ...
    private fun fetchStatus(): Status {
        val url = "${Constants.REST_URL}/$TABLE?select=data_json&sub_id=eq.$SUB_ID&limit=1"
        val resp = httpClient.newCall(
            Request.Builder().url(url).headers(readHeaders()).get().build()
        ).execute()
        val body = resp.body?.string() ?: "[]"
        if (!resp.isSuccessful) { Log.e(TAG, "fetchStatus HTTP ${resp.code}: $body"); return Status() }
        val arr = JSONArray(body)
        if (arr.length() == 0) return Status()
        val dj = arr.getJSONObject(0).optJSONObject("data_json") ?: return Status()
        return Status(
            startAt   = dj.longOrNull("startAt"),
            endAt     = dj.longOrNull("endAt"),
    ...
    private fun upsertStatus(s: Status) {
        ...
        val json = JSONObject().apply {
            ...
            put("app_id",     APP_ID);  put("data_type", "expiry")
            put("data_json",  dj);      put("status", "active")
            put("created_at", now);     put("updated_at", now)
        }
        val body = json.toString().toRequestBody("application/json; charset=utf-8".toMediaType())
        val url  = "${Constants.REST_URL}/$TABLE?on_conflict=sub_id"
        val resp = httpClient.newCall(
            Request.Builder().url(url).headers(upsertHeaders()).post(body).build()
        ).execute()
    ...
    private fun JSONObject.longOrNull(key: String): Long? {
        ...
        return when (val v = get(key)) {
            is Long   -> v; is Int    -> v.toLong()
            is Double -> v.toLong();  is Float -> v.toLong()
            is String -> v.toLongOrNull(); else -> null
        }
    }
