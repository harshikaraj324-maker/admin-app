...
import {
  ...
  Smartphone, Copy, CheckCircle, X, Key, Radio, Search, ChevronRight
} from "lucide-react";
...
export default function Apps() {
  ...
  // If an app is selected, show AppDetail
  if (selectedApp) {
    return (
      <AppDetail
        app={selectedApp}
  ...
  const handleToggle = async (e: React.MouseEvent, app: AdminApp) => {
    e.stopPropagation();
    try {
      await updateApp(app.id, { is_active: !app.is_active });
      setApps((p) => p.map((a) => a.id === app.id ? { ...a, is_active: !app.is_active } : a));
    } catch (ex: unknown) { alert(ex instanceof Error ? ex.message : "Error"); }
  };
  ...
  return (
    <div className="flex-1 overflow-auto h-full bg-[#080c16]">
      ...
      <div className="p-3 sm:p-6 space-y-3">
        ...
        {/* Create form */}
        {showForm && (
          <div className="bg-[#0d1220] border border-blue-500/25 rounded-2xl p-4 sm:p-5">
            ...
            <div className="mb-3">
              ...
              <input type="password" value={fPat} onChange={(e) => setFPat(e.target.value)}
                     placeholder="sbp_xxxxxxxxxxxx" autoComplete="off"
            ...
            {createErr && (
              <div className="text-xs text-red-400 bg-red-900/15 border border-red-800/30 rounded-xl px-4 py-3 mb-3">{createErr}</div>
            )}
        ...
        {loading && apps.length === 0 ? (
          ...
          <div className="text-center py-12 bg-[#0d1220] border border-slate-800 rounded-2xl">
            ...
            <button onClick={() => setSearch("")} className="text-xs text-blue-500 hover:text-blue-400 mt-1">Clear</button>
          </div>
        ) : apps.length === 0 ? (
                ...
                <div key={app.id} className="bg-[#0d1220] border border-slate-800/80 rounded-2xl overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="font-mono text-xs text-slate-500 tracking-wide">{app.token}</span>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {/* Realtime */}
                      <button
                        onClick={(e) => { e.stopPropagation(); setRtPatId(rtPatId === app.id ? null : app.id); setRtPat(getPat()); setRtMsg(null); }}
                        title="Enable Realtime"
                        className={`p-1.5 rounded-lg transition-colors ${rtPatId === app.id ? "bg-violet-700/40 text-violet-300" : "text-violet-500 hover:bg-violet-900/30"}`}
                      >
                        <Radio className="w-3.5 h-3.5" />
                      </button>
                      {/* Toggle */}
                      <button onClick={(e) => void handleToggle(e, app)} title={app.is_active ? "Disable" : "Enable"}
                              className={`p-1.5 rounded-lg transition-colors ${app.is_active ? "text-emerald-400 hover:text-red-400" : "text-slate-600 hover:text-emerald-400"}`}>
                        {app.is_active ? <Power className="w-3.5 h-3.5" /> : <PowerOff className="w-3.5 h-3.5" />}
                      </button>
                      {/* Constants.kt */}
                      <button onClick={(e) => { e.stopPropagation(); setKtId(showKt ? null : app.id); }} title="Constants.kt"
                              className={`p-1.5 rounded-lg transition-colors ${showKt ? "text-blue-400" : "text-slate-600 hover:text-white"}`}>
                        <Smartphone className="w-3.5 h-3.5" />
                      </button>
                      {/* Delete */}
                      <button onClick={(e) => void handleDelete(e, app.id)} title="Delete"
                  ...
                  {rtPatId === app.id && (
                    <div className="mx-3 sm:mx-4 mb-3 border border-violet-800/40 rounded-xl bg-violet-950/20 px-3 py-3">
                      <p className="text-xs text-violet-300 mb-2 font-medium">
                        PAT required to enable Realtime —{" "}
                      ...
                      <div className="flex gap-2">
                        <input type="password" value={rtPat} onChange={(e) => setRtPat(e.target.value)}
                               onKeyDown={(e) => e.key === "Enter" && void handleFixRealtime(app)}
                               placeholder="sbp_..." autoComplete="off" autoFocus
                        ...
                        <button onClick={() => void handleFixRealtime(app)} disabled={fixingRtId === app.id || !rtPat.trim()}
                          ...
                          {fixingRtId === app.id ? <span className="w-3 h-3 border border-white/30 border-t-white rounded-full animate-spin" /> : <Radio className="w-3 h-3" />}
                          Enable
                        </button>
                      </div>
                    </div>
                  ...
                  {rtMsg?.id === app.id && (
                    <div className={`mx-3 sm:mx-4 mb-3 px-3 py-2 rounded-xl text-xs font-medium flex items-center justify-between ${
                      rtMsg.ok ? "bg-violet-900/20 text-violet-300 border border-violet-800/40" : "bg-red-900/20 text-red-400 border border-red-800/40"
                    }`}>
                  ...
                  {/* Constants.kt */}
                  {showKt && (
                    <div className="border-t border-slate-800/60 bg-[#080c16] px-3 sm:px-4 py-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-slate-500 font-medium">Android — Constants.kt</span>
                        <button onClick={() => void copy(ktText, `kt-${app.id}`)}
                                className="flex items-center gap-1 text-xs text-slate-500 hover:text-white transition-colors">
                          {copiedId === `kt-${app.id}` ? <><CheckCircle className="w-3 h-3 text-emerald-400" />Copied!</> : <><Copy className="w-3 h-3" />Copy</>}
                        </button>
                      </div>
                      <pre className="bg-[#0d1117] border border-slate-800 rounded-xl p-3 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed">{ktText}</pre>
                    </div>
                  )}
                </div>
              );
